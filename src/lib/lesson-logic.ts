import { Exercise, MultipleChoice, WordBank, Typing, Listening, MatchPairs } from '@/types/schemas'
...
export function validateAnswer(exercise: Exercise, answer: any): ValidationResult {
...
function validateMultipleChoice(exercise: MultipleChoice, answer: number): ValidationResult {
...
function validateWordBank(exercise: WordBank, answer: string[]): ValidationResult {
    ...
    const isCorrect = userAnswerIndices.every((val, idx) => val === exercise.correctOrder[idx])
...
function validateListening(exercise: Listening, answer: string): ValidationResult {
...
function validateMatchPairs(exercise: MatchPairs, answer: Record<string, string>): ValidationResult {
    ...
    exercise.pairs.forEach((p: { left: string, right: string }) => correctMap.set(p.left, p.right))
